...
struct BSScript
{
    ...
    struct IsSkyrimTogetherExeRunningFunc : NativeFunction
    {
        using FunctionType = bool();

        IsSkyrimTogetherExeRunningFunc(const char* apFunctionName, const char* apClassName, FunctionType aFunction, Variable::Type aType);
