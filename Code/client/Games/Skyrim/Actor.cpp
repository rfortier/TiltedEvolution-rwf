...
bool TP_MAKE_THISCALL(HookSpeakSoundFunction, Actor, const char* apName, uint32_t* a3, uint32_t a4, uint32_t a5, uint32_t a6, uint64_t a7, uint64_t a8, uint64_t a9, bool a10, uint64_t a11, bool a12, bool a13, bool a14)
{
    ...
    static Actor* plastActor = nullptr;
    static char* plastName = nullptr;
    static steady_clock::time_point lastTime = steady_clock::now() -  100ms;
    static std::mutex serializeMutex;

    bool isDuplicate;
    {
        std::lock_guard<std::mutex> guard(serializeMutex);

        isDuplicate = plastActor == apThis && plastName == apName && steady_clock::now() - lastTime < 20ms;
        plastActor = apThis;
    ...
    auto isRemoteInScene = isRemote && apThis->GetCurrentScene() && apThis->GetCurrentScene()->isPlaying;
    ...
    if (isDuplicate)
        spdlog::debug(__FUNCTION__ ": suppressing duplicate DialogueEvent, formId {:X} isLeader {} name {}, apName {}",
    ...
    else
    {
        ...
        if (isRemoteInScene)
            spdlog::debug(__FUNCTION__ ": generating remote dialog event because in-scene formId {:X} isLeader {} name {}, apName {}",
                          apThis->formID, isLeader, apThis->baseForm->GetName(), apName);
    }
