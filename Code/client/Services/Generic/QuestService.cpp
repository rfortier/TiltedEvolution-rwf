                                     ...
                                     const EventDispatcher<TESQuestStartStopEvent>*)
{
    ...
    if (pQuest == nullptr || IsNonSyncableQuest(pQuest) || !m_world.Get().GetPartyService().IsInParty() ||
    ...
    spdlog::info(__FUNCTION__ ": {} {} formId: {:X}, questStage: {}, questType: {}, flags: {:X}, {} {}, name: {}",
                 startStop, miscQuest, apEvent->formId, pQuest->currentStage,
                 static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->flags, playerString,
...
BSTEventResult QuestService::OnEvent(const TESQuestStageEvent* apEvent, const EventDispatcher<TESQuestStageEvent>*)
{
    ...
    spdlog::info(__FUNCTION__ ": stage update {} formId: {:X}, questStage: {}, questType: {}, isStopped: {}, flags: "
                              "{:X}, {} {}, name: {}",
                 miscQuest, apEvent->formId, apEvent->stageId,
    ...
    const auto updateType =
        apEvent->stageId == 0 ? "reset" : "updated"; // An update to stage 0 is interpreted as a quest Reset. That
                                                     // SHOULD be true.FICME EXPERIMENT send Reset if this happens.

    // We should not see an Update to a started quest, unless it started a long time ago in StageWait.
    if (apEvent->stageId == 0 && (pQuest->flags & (TESQuest::Flags::Enabled | TESQuest::Flags::StageWait)) !=
                                     (TESQuest::Flags::Enabled | TESQuest::Flags::StageWait))
        spdlog::critical(__FUNCTION__ ":  suspected quest reset event, quest {} formId: {:X}, questStage: {}, "
                                      "questType: {}, isStopped: {}, flags {:X}, {} {}, name: {}",
                         updateType, apEvent->formId, apEvent->stageId,
                         static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                         pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());

    m_world.GetRunner().Queue([&, formId = apEvent->formId, stageId = apEvent->stageId, type = pQuest->type]() {
        GameId Id;
        auto& modSys = m_world.GetModSystem();
        if (modSys.GetServerModId(formId, Id))
        {
            ...
            update.Id = Id;
            update.Stage = stageId;
            // update.Status = stageId == 0 ? RequestQuestUpdate::Reset : RequestQuestUpdate::StageUpdate; // FIXME?
            // Experiment? Should it be checking IsStopped()? I think this is an indicator of StartsEnabled, so we never
            // see the start, not Reset
...
BSTEventResult QuestService::OnEvent(const TESSceneEvent* apEvent, const EventDispatcher<TESSceneEvent>*)
{
    ...
    spdlog::info(__FUNCTION__ "::TESSceneEvent*"
                              ": sending scene {}, isPlaying {} formId: {:X} {} formId: {:X}, questStage: {}, "
                              "questType: {}, isStopped: {}, flags: {:X}, {} {}, name: {}",
                 beginEnd, isPlaying, apEvent->sceneFormId, miscQuest, pQuest->formID, pQuest->currentStage,
                 static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(), pQuest->flags,
                 playerString, PlayerId(), pQuest->fullName.value.AsAscii());
    ...
    // TODO: EXPERIMENT
    m_world.GetRunner().Queue([&, sceneId = apEvent->sceneFormId, questId = pQuest->formID,
                               stageId = pQuest->currentStage, type = pQuest->type, sceneType = apEvent->sceneType]() {
        GameId sceneGameId;
        GameId questGameId;
        auto& modSys = m_world.GetModSystem();
...
BSTEventResult QuestService::OnEvent(const TESSceneActionEvent* apEvent, const EventDispatcher<TESSceneActionEvent>*)
{
    ...
    spdlog::info("TESSceneActionEvent: questId {:X}, scene id {:X}, action index {}, {} {}", apEvent->questFormId,
...
BSTEventResult QuestService::OnEvent(const TESScenePhaseEvent* apEvent, const EventDispatcher<TESScenePhaseEvent>*)
{
    ...
    spdlog::info("TESScenePhaseEvent event: quest {:X}, stage {}, scene {:X}. phase index {}, type {}, "
                 "isAnyScenePlaying {},by {} {}",
                 pQuest ? pQuest->formID : 0, apEvent->questStageId, apEvent->sceneFormId, apEvent->phaseIndex,
                 sceneType, isAnyScenePlaying, playerType, PlayerId());
    ...
    // is Phase 0 End.
    // And on very long scenes, refreshes SceneMaster so it doesn't time out.
...
void QuestService::OnQuestUpdate(const NotifyQuestUpdate& aUpdate) noexcept
{
    ...
    if (pQuest == nullptr || !m_world.Get().GetPartyService().IsInParty())
        return; // pQuest == nullptr shouldn't happen.
    ...
    //
    // To solve the conflicts, the initiator of the scene is remembered as SceneMaster,
    // which is used in logs and some experiments.
    //
    // Update logic is if not in scene, accept any update you get. This supports quests
    // that rewind the stage without a reset. 
    // 
    // If you ARE in a scene, only accept advancing quest stage changes. With everyone 
    // playing the same scene very close together, there are some network-lag windows that 
    // can get past the dedup logic, So ignore rewind.
    // 
    ...
    // unnecessary now that we have fixed scene playing speed sync to be better.
    //
    bool updateDisabled = pQuest->IsAnyCutscenePlaying() &&
        (aUpdate.Status == NotifyQuestUpdate::StageUpdate && aUpdate.Stage < pQuest->currentStage);
    ...
    case NotifyQuestUpdate::Started:
        if (isRunning)
        {
            spdlog::info(__FUNCTION__ ": suppressing duplicate start {} gameId: {:X}, formId: {:X}, questStage: {}, "
                                      "questType: {}, isStopped: {}, flags: {:X}, {} {}, name: {}",
                         miscQuest, aUpdate.Id.LogFormat(), formId, aUpdate.Stage,
                         static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                         pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());
        }
        else
        {
            spdlog::info(__FUNCTION__ ": remotely started {} start gameId: {:X}, formId: {:X}, questStage: {}, "
                                      "questType: {}, isStopped: {}, flags: {:X}, {} {}, name: {}",
                         miscQuest, aUpdate.Id.LogFormat(), formId, aUpdate.Stage,
                         static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                         pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());
    ...
    case NotifyQuestUpdate::Reset:
        spdlog::info(__FUNCTION__ ": remotely reset {} gameId: {:X}, formId: {:X}, questStage: {}, questType: {}, "
                                  "isStopped: {}, flags: {:X}, {} {}, name: {}",
                     miscQuest, aUpdate.Id.LogFormat(), formId, aUpdate.Stage,
                     static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                     pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());
    ...
    case NotifyQuestUpdate::StageUpdate: // TODO? Insert START iff needed?
        spdlog::info(__FUNCTION__ ": remotely updated {} gameId: {:X}, formId: {:X}, questStage: {}, questType: {}, "
                                  "isStopped: {}, flags: {:X}, {} {}, name: {}",
                     miscQuest, aUpdate.Id.LogFormat(), formId, aUpdate.Stage,
                     static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                     pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());
    ...
    case NotifyQuestUpdate::Stopped:
        spdlog::info(__FUNCTION__ ": remotely stopped {} gameId: {:X}, formId: {:X}, questStage: {}, questType: {}, "
                                  "isStopped: {}, flags: {:X}, {} {}, name: {}",
                     miscQuest, aUpdate.Id.LogFormat(), formId, aUpdate.Stage,
                     static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                     pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());
    ...
    default:
        break;
        spdlog::error(__FUNCTION__ ": unknown remote status {} {} gameId: {:X}, formId: {:X}, questStage: {}, "
                                   "questType: {}, isStopped: {}, flags: {:X}, {} {}, name: {}",
                      aUpdate.Status, miscQuest, aUpdate.Id.LogFormat(), formId, aUpdate.Stage,
                      static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                      pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());
    }

    if (!wasUpdated)
        spdlog::error(__FUNCTION__ ": failed to remotely update status {} {} gameId: {:X}, formId: {:X}, questStage: "
                                   "{}, questType: {}, isStopped: {}, flags: {:X}, {} {}, name: {}",
                      aUpdate.Status, miscQuest, aUpdate.Id.LogFormat(), formId, aUpdate.Stage,
                      static_cast<std::underlying_type_t<TESQuest::Type>>(pQuest->type), pQuest->IsStopped(),
                      pQuest->flags, playerString, PlayerId(), pQuest->fullName.value.AsAscii());
...
void QuestService::OnQuestSceneUpdate(const NotifyQuestSceneUpdate& aUpdate) noexcept
{
    ...
    if (!pQuest)
    {
        spdlog::error(__FUNCTION__ ": failed to find questId: {:X}, {} {}", aUpdate.QuestId.LogFormat(), playerType,
                      PlayerId());
        return;
    }
    if (!pScene)
    {
        spdlog::error(__FUNCTION__ ": failed to find sceneId: {:X}, {} {}", aUpdate.SceneId.LogFormat(), playerType,
                      PlayerId());
        return;
    }
    ...
    {
        if (pScene->isPlaying)
            spdlog::info(__FUNCTION__ ": skip starting scene already playing, questId {:X}, sceneId {:X}, {} {}",
                         aUpdate.QuestId.LogFormat(), aUpdate.SceneId.LogFormat(), playerType, PlayerId());
        else
        {
            spdlog::error(__FUNCTION__ ": FIXME DISABLED starting scene questId {:X}, sceneId {:X}, {} {}",
                          aUpdate.QuestId.LogFormat(), aUpdate.SceneId.LogFormat(), playerType, PlayerId());
    ...
    {
        if (!pScene->isPlaying)
            spdlog::info(__FUNCTION__ ": skip stopping scene not playing, questId {:X}, sceneId {:X}, {} {}",
                         aUpdate.QuestId.LogFormat(), aUpdate.SceneId.LogFormat(), playerType, PlayerId());
        else
        {
            spdlog::error(__FUNCTION__ ": stopping scene questId {:X}, sceneId {:X}, {} {}",
                          aUpdate.QuestId.LogFormat(), aUpdate.SceneId.LogFormat(), playerType, PlayerId());
            pScene->ScriptStop();
        }
    }
