...
TaskDialog::TaskDialog(HICON aIcon, const wchar_t* apTitle, const wchar_t* apMessage, const wchar_t* apMoreContext, const wchar_t* apOptionalDetails)
    : m_iconHandle(aIcon)
    , m_pWindowTitle(apTitle)
    , m_pMessage(apMessage)
    , m_pDetails(apOptionalDetails)
...
HRESULT CALLBACK TaskDialog::TaskDialogCallbackProc(HWND aHwnd, UINT notificationCode, WPARAM w_param, LPARAM lParam, LONG_PTR ref_data)
{
    ...
    case TDN_CREATED:
        ...
        break;
    case TDN_HYPERLINK_CLICKED: ShellExecuteW(nullptr, L"open", (LPCWSTR)lParam, nullptr, nullptr, SW_NORMAL); break;
    case TDN_TIMER:
    {
...
int TaskDialog::Show(int aIconId)
{
    ...
    std::transform(m_buttonList.begin(), m_buttonList.end(), std::back_inserter(buttons), [](const std::pair<int, std::wstring>& button) -> TASKDIALOG_BUTTON { return {button.first, button.second.c_str()}; });
