...
void QuestService::OnQuestChanges(const PacketEvent<RequestQuestUpdate>& acMessage) noexcept
{
    ...
    auto questIt =
        std::find_if(entries.begin(), entries.end(), [&message](const auto& e) { return e.Id == message.Id; });
    ...
    else
    {
        ...
        spdlog::info("{}: started/discovered quest: {:X}, stage: {}, by {} {:X}", __FUNCTION__, message.Id.LogFormat(),
                     notify.Stage, playerTypeString, pPlayer->GetId());
    }
    ...
    case RequestQuestUpdate::StageUpdateForced:
        useAggressiveSend = true;

        [[fallthrough]];
    case RequestQuestUpdate::StageUpdate:
        notify.Status = NotifyQuestUpdate::StageUpdate;
        spdlog::info("{}: updated quest: {:X}, stage: {}, SceneMaster {}, by {} {:X}", __FUNCTION__,
                     message.Id.LogFormat(), notify.Stage, notify.SceneMaster, playerTypeString, pPlayer->GetId());
        break;
    ...
    case RequestQuestUpdate::Stopped:
        ...
        {
            spdlog::warn("{}: unable to delete quest object {:X} (already stopped or first update is stopped)",
                         __FUNCTION__, notify.Id.LogFormat());
        }
        break;
    ...
    case RequestQuestUpdate::Reset:
        ...
        spdlog::info("{}: reset quest: {:X}, stage: {}, by {} {:X}", __FUNCTION__, message.Id.LogFormat(), notify.Stage,
                     playerTypeString, pPlayer->GetId());
        break;

    default:
        spdlog::error("{}: unknown quest status {} quest: {:X}, stage: {}, status {}, by {} {:X}", __FUNCTION__,
                      notify.Status, message.Id.LogFormat(), notify.Stage, notify.Status, playerTypeString,
                      pPlayer->GetId());
        break;
    }
    ...
    if (inParty)
    {
        if (notify.ClientQuestType == 0 ||
            notify.ClientQuestType == 6) // Types None or Miscellaneous. Hard-coded to avoid client header file.
        {
            if (!bEnableMiscQuestSync)
                return;

            spdlog::info("{}: syncing type none/misc quest to party, quest: {:X}, questStage: {}, questStatus: {}, "
                         "questType: {}",
        ...
        if (isLeader)
        {
            ...
            if (!useAggressiveSend &&
                dedupHistory.FoundWPlayerId(notify.Id, notify.Stage, notify.Status, pPlayer->GetId()))
                spdlog::info("{}: SendToParty dropping duplicate: quest: {:X}, stage: {}, status: {}, by {} {:X}",
                             __FUNCTION__, notify.Id.LogFormat(), notify.Stage, notify.Status, playerTypeString,
                             pPlayer->GetId());
            else
            {
                spdlog::info("{}: SendToParty: quest: {:X}, stage: {}, status: {}, by {} {:X}", __FUNCTION__,
                ...
                for (Player* pPlayer : m_world.GetPlayerManager())
                {
                    ...
                    if (partyComponent.JoinedPartyId == partyId)
                    {
                        if (!useAggressiveSend &&
                            dedupHistory.FoundWPlayerId(notify.Id, notify.Stage, notify.Status, pPlayer->GetId()))
                        {
                            spdlog::info("{}: SendToParty skipping duplicate send quest: {:X}, stage: {}, status: {}, "
                                         "SceneMaster {}, to player {:X}",
                                         __FUNCTION__, notify.Id.LogFormat(), notify.Stage, notify.Status,
                                         notify.SceneMaster, pPlayer->GetId());
                        }
                        else
                        {
                            spdlog::info("{}: SendToParty sending quest: {:X}, stage: {}, status: {}, SceneMaster {}, "
                                         "to player {:X}",
                                         __FUNCTION__, notify.Id.LogFormat(), notify.Stage, notify.Status,
                                         notify.SceneMaster, pPlayer->GetId());
                            pPlayer->Send(notify);
        ...
        else
        {
            ...
            if (bFound)
                spdlog::info("{}: SendToLeader dropping duplicate quest: {:X}, stage: {}, status: {}, by {} {:X}",
                             __FUNCTION__, notify.Id.LogFormat(), notify.Stage, notify.Status, playerTypeString,
                             pPlayer->GetId());
            else
            {
                spdlog::info("{}: SendToLeader quest: {:X}, stage: {}, status: {}, by {} {:X}", __FUNCTION__,
                             notify.Id.LogFormat(), notify.Stage, notify.Status, playerTypeString, pPlayer->GetId());
...
void QuestService::OnQuestSceneChanges(const PacketEvent<RequestQuestSceneUpdate>& acMessage) noexcept
{
    ...
    {
        bool needSceneMaster = dedupHistory.NeedSceneMaster() ||
                               !dedupHistory.Found(message.SceneId, 0, RequestQuestUpdate::StatusCode::SceneUpdate);
        ...
        if (!needSceneMaster)
            spdlog::info("{}: quest {:X}, scene {:X}, sceneType {}, {} {} already have SceneMaster player {}",
                         __FUNCTION__, message.QuestId.LogFormat(), message.SceneId.LogFormat(), sceneTypeString,
                         playerTypeString, pPlayer->GetId(), dedupHistory.GetSceneMaster());
        else
        {
            dedupHistory.SetSceneMaster(pPlayer->GetId());
            spdlog::info("{}: quest {:X}, scene {:X}, sceneType {}, {} {} is now SceneMaster", __FUNCTION__,
                         message.QuestId.LogFormat(), message.SceneId.LogFormat(), sceneTypeString, playerTypeString,
                         pPlayer->GetId());

            spdlog::info("{}: sending scene {} update quest {:X}, scene {:X}, by {} {}", __FUNCTION__, sceneTypeString,
                         notify.QuestId.LogFormat(), notify.SceneId.LogFormat(), playerTypeString, pPlayer->GetId());
    ...
    else // END scene
    {
        if (dedupHistory.GetSceneMaster() == pPlayer->GetId())
        {
            ...
            spdlog::info("{}: quest {:X}, scene {:X}, scenetype {}, {} {} removing SceneMaster player {}", __FUNCTION__,
                         message.QuestId.LogFormat(), message.SceneId.LogFormat(), sceneTypeString, playerTypeString,
                         pPlayer->GetId(), dedupHistory.GetSceneMaster());
            //dedupHistory.ResetSceneMaster();

            // Force an update out to bring anyone behind to current quest stage.
            spdlog::info(
                "{}: SceneMaster {} {} sending current quest stage {} to party quest {:X}, scene {:X}, scenetype {}",
                __FUNCTION__, playerTypeString, pPlayer->GetId(), message.Stage, message.QuestId.LogFormat(),
                message.SceneId.LogFormat(), sceneTypeString);
